...
:clapper: 但是使用立即调用函数表达式包裹代码后，不方便调试（在开发者工具无法直接访问 Vue 实例 `vm`），可以在开发时将立即调试函数表达式注释，上线代码时再包裹。
...
:bulb: 使用指令 `v-on` 设置的事件监听会**在 ViewModel 销毁后事件处理器都会自动被删除**，但是使用自己使用 `addEventlistener` 等方法设置的事件监听，记得需要在 `beforeDestroy()` 生命周期函数中手动解绑，以便释放效能。
