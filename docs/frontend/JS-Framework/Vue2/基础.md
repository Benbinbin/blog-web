...
* 在外部（引用该组件时，在组件的标签上）通过指令 `v-model` 双向绑定需要同步的变量，它的数据作为 prop 传递到组件内；:bulb: 记得在组件内部需要在 `props` 选项里声明这个 prop，可能是 `checked` 或 `value`，视组件内部使用的表单类型。
* 在外部（引用该组件时，在组件的标签上）同时监听组件抛出的自定义事件，并接收其抛出的数据，然后在事件处理函数中修改双向绑定的变量的值

* 在组件内使用指令 `v-bind` 将表单元素的值绑定到 prop 变量上，可能是 `checked` 或 `value`，视组件内部使用的表单类型（**这样就可以将外部传进来的值作为表单的值，实现与外部数据的「同步」）**

* 在组件内使用指令 `v-on` 监听表单元素的 `input` 或 `change` 等事件，并通过抛出自定义事件 `$emit('cutom-input'， $event.target.value)` 向外传递相应的数据
