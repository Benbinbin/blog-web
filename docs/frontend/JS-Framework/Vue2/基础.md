...
过滤器应该被添加**在 JavaScript 表达式的尾部**，由**「管道」符号**指示 `{{ value | filterName }}`，:clapper: 过滤器是 JavaScript 函数，因此可以接收参数，总是接收管道号之前的 JavaScript 表达式的值作为第一个参数，主动传入的数据则作为第二个参数开始传递。
...
在创建 Vue 实例之前可以使用 Vue 原型 prototype 中的方法 `filter` 定义全局过滤器
