...
## 对象
在 TypeScript 中使用 ==关键字**接口 Interfaces** 来定义对象的类型，类似于数据库的 scheme，用于描述对象的「形状」 Shape==，接口的名称一般首字母大写。
...
:bulb: 一般而言，数组 Array 是指用于存储**数据类型相同**的一系列元素的一种数据结构，虽然可以使用 `any[]` 来支持格元素是任意数据类型，但不推荐；而[元组 Tuple](#元组) 则是用于存储**数据类型不同**的一系列元素的一种数据结构。
...
元组 Tuple 是一种与数组类似的数据类型，但它各项元素可以是**不同数据类型**，使用 `[type1, type2]` 约束元组中各项的数据类型
...
在 TypeScript 中对函数进行约束时，需要把**输入**和**输出**都考虑到:
* 输入的约束在入参中设置
* 输出的约束在函数名（参数）后设置
...
给变量约束为特定的类的数据类型，其中最简单的是直接在 `:` 后使用类名即可
