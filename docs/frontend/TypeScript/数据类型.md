...
:bulb: 在 TypeScript 中**在变量后使用 `:` 指定以上 JavaScript 内置的类型**，`:` 的前后有没有空格都可以。此外对于函数，还可以同时约束输入和输出的数据类型，其中 [`void` 类型](#空值)就是针对函数无返回值的情况
...
事实上 `new Boolean()` 返回的是一个 `Boolean` 对象，直接调用 `Boolean` 也可以返回一个 `boolean` 类型。类似地，其他基本类型（除了 `null` 和 `undefined`）也是一样。
...
字符串字面量类型用来**约束取值**只能是**某几个字符串中的一个**。
...
JavaScript 没有空值（Void）的概念，在 TypeScript 中使用 `void` 表示**没有任何返回值的函数**
...
在 TypeScript 中使用 `Any` 来表示变量允许赋值为任意类型，则变量允许在上下文中被赋值为任意类型，因为 `any` 类型完全不被类型检查
...
:bulb: 声明一个变量为任意值之后，对它的任何操作，返回的内容的类型都是任意值，因此可以在该变量上访问任何属性或调用任何方法，在 ts 编译为 js 阶段都不会报错。

:bulb: 变量如果在声明的时未指定其类型，且未设定初始值，那么它会被识别为任意值类型
...
在 TypeScript 中使用**接口 Interfaces** 来定义对象的类型，类似于数据库的 scheme 用于描述对象的「形状」 Shape，接口的名称一般首字母大写，*有的编程语言中会[建议接口的名称加上 `I` 前缀](https://docs.microsoft.com/en-us/previous-versions/dotnet/netframework-1.1/8bc1fexb(v=vs.71))*。
...
:bulb: 在面向对象语言中，接口 Interfaces 是对行为的抽象，而具体如何行动需要由类 classes 去实现 implement。TypeScript 的接口 interfaces 还用于对**类**的一部分行为进行抽象。
...
如果希望不要完全匹配一个接口的形状，那么可以在定义接口时用**可选属性**，在属性名后用 `?` 标注，即该属性在对象中可以不存在，但**仍然不允许添加未定义的属性**
...
如果希望一个接口更灵活有更大的可拓展性，可以在定义接口时添加**任意属性**，一个接口中只能定义**一个**任意属性
...
以上示例中使用 `[propName: string]` 定义了一个任意属性（属性名是 `String` 类型，属性值是任意值）

:warning: 一旦定义了任意属性，那么接口中其他的**确定属性**和**可选属性**的数据类型都必须是**该任意属性的数据类型的子集**。因此如果接口中有多种不同数据类型的属性，那么在定义任意属性时应该使用[联合类型](#联合类型)
...
以上示例中任意属性的值允许是 `string`，但是可选属性 `age` 的值却是 `number`，`number` 不是 `string` 的子属性，所以报错了。
...
:bulb: 只读的**约束是指只能够在第一次给对象赋值的时候才能设置只读属性（之后就不能重新赋值修改了）**，而不是第一次给只读属性赋值的时候
...
:bulb: 一般而言，**数组 Array**是指用于存储**相同类型**的项的一种数据结构，虽然可以使用 `any[]` 来支持格元素是任意数据类型，但不推荐；而**[元组 Tuple](#元组)**则是用于存储**不同类型**的项的一种数据结构。

### 类型+方括号

最简单的方法是使用 **类型`[]`** 来定义**`数组`**类型及其中的**元素的类型**
...
在 TypeScript 中**不**能用普通的 **类型`[]`** 方式来定义其类型，而需要使用接口的方式来定义。因为类数组 Array-like Object 不是数组类型，比如 `arguments`（函数参数对象）就是一个类数组对象。
...
:bulb: 事实上 TypeScript 为常用的类数组对象内置了接口定义，如 `IArguments`, `NodeList`, `HTMLCollection` 等，因此以上的定义可以简化为
...
元组 Tuple 与数组类似的数据类型，但它各项元素可以是**不同数据类型**，使用 `[type1, type2]` 约束元组中各项的数据类型
...
:bulb: 允许添加额外的元素，但是「越界」元素的类型会被**限制为元组中各项类型的联合类型**：
...
* 常数项 constant member
    ...
    * 不具有初始化函数并且之前的枚举成员是常数。第一个枚举元素初始值的默认为 `0`，后一个枚举成员的值上一个枚举成员的值加 `1`。
...
在 TypeScript 中对函数进行约束时，需要把**输入**和**输出**都考虑到。其中输入的约束在入参中设置，输出的约束在函数名（参数）后设置。
...
:bulb: 不要混淆了 TypeScript 中的 `=>` 和 ES6 中的 `=>`，在 TypeScript 的类型定义中 **`=>` 用来表示函数的定义**，左边是输入类型，需要用括号 `()` 括起来，右边是输出类型。
...
:bulb: 在 ES6 中允许给函数的参数添加默认值，TypeScript 会将添加了默认值的参数识别为**可选参数**，但这种情况下，**不**受「可选参数必须接在必需参数后面」的限制
...
对于**函数表达式**还可以使用接口的方式来约束变量可以通过赋值指向的函数的「形状」，即对等号左侧的变量进行类型限制，可以保证以后对函数名赋值时保证参数个数、参数类型、返回值类型不变。

接口对象的属性（左侧）需要用括号 `()` 括起来，表示输入类型；属性值（右侧）是输出类型
...
在 ES6 中使用关键字 `class` 来定义类，使用关键字 `new` 来实例化。而 TypeScript 除了实现了所有 ES6 中的类的功能以外，还添加了一些新的用法。

给变量加上类的数据类型，其中最简单的是直接在 `:` 后使用类名即可
...
:bulb: 修饰符还可以直接应用中构造函数的参数中，使代码更简洁，等同于类中定义该属性同时给该属性赋值
...
只读属性通过关键字 `readonly` 进行设置，只允许出现在属性声明或索引签名或构造函数中。
...
关键字 `abastract` 用于定义抽象类和抽象方法（类似于占位符，没有写实际的实现逻辑代码）

:bulb: 抽象类是**不**允许被实例化的，只能用于被继承。其中抽象方法必须被子类实现。
...
接口 Interfaces 除了可用于描述[对象的「形状」shape](#对象)，还可以对类的一部分行为进行抽象。有时候不同类之间可以有一些共有的特性，这时候就可以把特性提取成接口 `interfaces`，然后在需要该功能的类中通过 `implements` 关键字来**实现**。通过这个特性可以复用一些类的约束条件。
...
在 TypeScript 中支持将变量设定为**联合类型 Union Types**，表示取值可以是多种类型中的一种，每个数据类型之间使用 `|` 分隔
...
以上示例中 `let myFavoriteNumber: string | number` 的含义是，允许 `myFavoriteNumber` 的类型是 `string` 或者 `number`，但是**不**能是其他类型。

:warning: 在编译时，如果 TypeScript 不确定一个联合类型的变量到底是哪个类型（一般是在针对函数形参的情况），所以只能访问这些类型里**共有的属性或方法**，否则编译会保持
...
以上示例创建了一个函数 `createArray`，它的作用是创建一个指定长度的数组。**在函数名后添加了 `<T>`**，其中 `T` 用来指代任意输入的类型，然后在后面就可以使用了，在输入参数中 `value: T` 和输出 `Array<T>` 中，还有函数内的变量 `result: T[]`。接着在调用的代码中指定它具体的类型为 `string`，主要就可以对相应的参数和变量进行**具体的数据类型约束**。
...
接口是描述函数或类一部分的数据类型的「形状」，其实接口可以使用泛型，以便接口所描述的数据类型「形状」更通用
